...
class PchipSpline {
  ...
  PchipSpline(this._x, this._y) {
    assert(_x.length == _y.length && _x.length >= 2,
        'Need ≥ 2 knots for PCHIP');
    _computeSlopes();
  }
  ...
  void _computeSlopes() {
    ...
    final h = List<double>.generate(n - 1, (i) => _x[i + 1] - _x[i]);
    final delta =
        List<double>.generate(n - 1, (i) => (_y[i + 1] - _y[i]) / h[i]);

                                                   
    for (int i = 1; i < n - 1; i++) {
      if (delta[i - 1].sign != delta[i].sign ||
          delta[i - 1] == 0 ||
          delta[i] == 0) {
        ...
        final w1 = 2.0 * h[i] + h[i - 1];
        final w2 = h[i] + 2.0 * h[i - 1];
        _d[i] = (w1 + w2) / (w1 / delta[i - 1] + w2 / delta[i]);
      }
    }

                                                          
    _d[0] = _endpointSlope(h[0], h[1], delta[0], delta[1]);
    _d[n - 1] =
        _endpointSlope(h[n - 2], h[n - 3], delta[n - 2], delta[n - 3]);

                                        
    if (_d[0].sign != delta[0].sign) _d[0] = 0.0;
    if (_d[n - 1].sign != delta[n - 2].sign) _d[n - 1] = 0.0;
  }

                                                      
  double _endpointSlope(double h0, double h1, double d0, double d1) {
    final s = ((2.0 * h0 + h1) * d0 - h0 * d1) / (h0 + h1);
                              
    if (s.sign != d0.sign) return 0.0;
    if (d0.sign != d1.sign && s.abs() > 3.0 * d0.abs()) {
      return 3.0 * d0;
    }
    return s;
  }
  ...
  double evaluate(double x) {
    final int i = _interval(x);
    final double h = _x[i + 1] - _x[i];
    if (h.abs() < 1e-15) return _y[i];

    final double t = (x - _x[i]) / h;
    final double t2 = t * t;
    ...
    return h00 * _y[i] + h10 * h * _d[i] + h01 * _y[i + 1] + h11 * h * _d[i + 1];
  }
...
(List<double>, List<double>) pchipInterpolate(
  List<double> times,
  List<double> values,
  double fs,
) {
  ...
  final int nOut = ((times.last - times.first) / dt).floor() + 1;
  final newT = List<double>.generate(nOut, (i) => times.first + i * dt);
  final spline = PchipSpline(times, values);
  return (newT, spline.evaluateList(newT));
}
